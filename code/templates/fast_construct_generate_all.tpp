...
std::unordered_map<std::vector<size_t>, lemon::ListGraph::Node, Hasher> generate_all(const std::vector<std::vector<std::uint64_t>>& sigs, const std::vector<std::pair<size_t,size_t>>& lvls,lemon::ListGraph& graph){
    ...
    construct_graph(con_comps, graph, labMap);

    for(const std::vector<size_t>& comp : con_comps)
        recursive_step(sigs, graph, labMap, comp, lvls, 1);
    return labMap;
}